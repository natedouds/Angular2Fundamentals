...
export class EventService {
    ...
    getEvents(): Observable<IEvent[]> {
        ...
        //map lets you take pieces of data of 1 type, and turn it into a data of a different type
        //note: neither .get or .map actually make the http call; it's not until someone subscribes to the event
        return this.http.get("/api/events").map((response: Response) => {
            return <IEvent[]>response.json();//map it to array of IEvent
        })
        //take the mapped response and handle errors
